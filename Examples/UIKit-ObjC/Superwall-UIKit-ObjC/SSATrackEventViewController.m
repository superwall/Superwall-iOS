...
- (IBAction)trackEvent:(id)sender {
    ...
    presentationStyleOverride:PaywallPresentationStyleNone
                       onSkip:^(enum SWKPaywallSkippedReason reason, NSError * _Nullable error) {
        [weakSelf paywallSkippedWithReason:reason error:error];
    }
                    onPresent:^(PaywallInfo * _Nonnull paywallInfo) {
        [weakSelf paywallPresentedWithPaywallInfo:paywallInfo];
    }
                    onDismiss:^(enum SWKPaywallDismissedResultState dismissedResultState, NSString * _Nullable productIdentifier, PaywallInfo * _Nonnull paywallInfo) {
...
- (void)paywallPresentedWithPaywallInfo:(PaywallInfo *)paywallInfo {
    NSLog(@"Paywall info is %@", paywallInfo);
}

- (void)paywallDismissedWithResultState:(enum SWKPaywallDismissedResultState)dismissedResultState productIdentifier:(nullable NSString *)productIdentifier paywallInfo:(PaywallInfo *)paywallInfo {
