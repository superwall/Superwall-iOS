...
static inline PurchaseResult PurchaseResultFromTransactionState(SKPaymentTransactionState state, NSError *error) {
    switch (state) {
...
@interface SSASuperwallService () <SuperwallDelegate>
...
- (void)purchaseWithProduct:(SKProduct * _Nonnull)product completion:(void (^ _Nonnull)(enum PurchaseResult, NSError * _Nullable))completion { 
    [[SSAStoreKitService sharedService] purchaseProduct:product withCompletion:^(SKPaymentTransactionState state, NSError * _Nullable error) {
        ...
        PurchaseResult purchaseResult = PurchaseResultFromTransactionState(state, error);
