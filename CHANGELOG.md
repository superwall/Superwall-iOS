...
- The Customer Center now shows prices for subscriptions bought on the web (Stripe, Paddle). StoreKit can't resolve a web product, so those cards previously rendered with no price; the price is now read from the Superwall product catalogue when StoreKit returns nothing. Titles are derived from the product identifier (`web_pro_monthly` shows as "Web Pro Monthly") until the catalogue returns a display name.
