...
- Superwall automatically handles all subscription-related logic, meaning that it's no longer a requirement to implement any of the delegate methods. Note that if you're using RevenueCat, you will still need to use the delegate methods. This is because the Superwall-handled subscription status is device-specific, whereas RevenueCat is user-specific. If you don't mind about whether subscription status is user-specific, you can just set RevenueCat in observer mode and we'll take care of the rest :)
